...
class GeminiService {
    constructor(apiKey) {
        this._genAI = null; // Private instance of the client
        this._model = null;
        this._apiKey = null;
        this._currentModelName = null; // To track the current active model
    ...
    _createGenerativeModel(modelName, config) {
     ...
     */
    async _fetchAndSelectFallbackModel() {
        if (!this._apiKey) {
     ...
     */
    async _handleGeminiRequest(requestFn, fallbackConfig) {
        if (!this._model) {
            throw new Error(GEMINI_API_KEY_ERROR_MESSAGE);
        }
        try {
    ...
    _constructPrompt(userPrompt, systemPrompt, isJsonRequest = false) {
        let finalPrompt = '';
        if (systemPrompt) {
    ...
    async *generateStream(prompt, systemPrompt) {
        ...
        const streamRequest = async (model) => {
